      ...
      <div className="relative z-10 max-w-5xl xl:max-w-6xl mx-auto px-4 sm:px-6 w-full py-6">
            ...
            <CardPitch accentColor="#5170ff">
              ...
              <div className="rounded-lg overflow-hidden border border-[var(--border)]">
                {roClasses.map((c, idx) => (
                  <div
                    key={c.name}
                    className={`flex items-center justify-between px-3 py-2.5 ${idx % 2 === 0 ? 'bg-transparent' : 'bg-[var(--muted)]'}`}
                  >
            ...
            <CardPitch accentColor="#14b8a6">
              ...
              <div className="rounded-lg overflow-hidden border border-[var(--border)]">
                {ctTypes.map((c, idx) => (
                  <div
                    key={c.name}
                    className={`flex items-center justify-between px-3 py-2 ${idx % 2 === 0 ? 'bg-transparent' : 'bg-[var(--muted)]'}`}
                  >
                    <div className="flex items-center gap-2.5">
                    ...
                    {c.ifv != null ? (
                      ...
                      <span className="font-mono text-[10px] text-[var(--muted-foreground)]">&mdash;</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        <FragmentReveal delay={0.35}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 flex gap-3 items-start">
